...
1) displaying combining ingredients string using a loop
2) calculate the cost of a pie
3) calculate the total revenue 
*/

// calculates cost of a single pie having one responsibility
function costOfAPie(recipe) {
...
function totalRevenue(totalCost, pieQuantity, profitMargin){
  let revenue = totalCost * (profitMargin || 1.2);
  console.log(`Sold ${pieQuantity} pies for $${revenue.toFixed(2)}!`);
}

// prints the cake baking progress
function combineIngredients(pieType, pieQuantity, recipe){
...
function bakeAndSellPies(pieType, pieQuantity, profitMargin) {
  ...
  // 1) displaying combining ingredients string using a loop
  combineIngredients(pieType, pieQuantity, recipe)

  // // Print the cost of each pie based on the cost of each ingredient
  const costOfPie = costOfAPie(recipe)
